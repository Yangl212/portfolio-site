...
export const projects = {
  ...
  lastmessage: {
    ...
    /* Three stills off the game's own case-video loops: the chat, the
       ...
       thrown out of the folder like evidence cards. */
    cardStyle: "toss",
  ...
  vortexnet: {
    ...
    /* Three screens from the shipped dashboard, the same "whole screens
       from the interface" treatment as cleared's. */
    cardStyle: "windows",
    cards: ["/vortexnet/media/dashboard-detail.webp", "/vortexnet/media/nav-after.webp", "/vortexnet/media/metrics-after.webp"]
  },
  cleared: {
    title: "AI Calendar",
    navName: "AI Calendar",
...
/* About is only written for the UI/UX track so far. The visual track's
   name still prints when it is pressed - that part needs nothing from
   this page - it just does not go anywhere afterward, and the header's
   own mark is not a link either, until there is a page for it to open. */
export function aboutReady(track) {
  return track !== "visual"
}
