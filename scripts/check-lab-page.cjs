...
;(async () => {
  ...
  try {
    ...
    const originalCaptions = page.locator('[data-lab-tile="0:0"] figure[data-lab-item] figcaption')
    assert.equal(await originalCaptions.count(), 12, 'Every original image has a compact caption')
    assert(await originalCaptions.evaluateAll(captions => captions.every(caption => caption.querySelector('h2')?.textContent.trim() && caption.querySelector('p')?.textContent.trim())), 'Every image caption includes a title and short description')
    ...
    await page.mouse.up()
    assert.equal(await viewport.evaluate(element => document.activeElement === element), false, 'Mouse dragging does not add the canvas focus outline beside the header border')
